...
def pad_features(features, target_length=None):
    """
    Pads or truncates feature arrays to ensure all have the same number of frames.

    Parameters:
    -----------
    features : list of np.ndarray
        A list of feature arrays, each with shape (n_frames, n_features).

    target_length : int, optional
        The length to which each feature array should be padded or truncated.
        If None, the maximum length in the features list will be used.

    Returns:
    --------
    np.ndarray
        A 3D array of shape (n_samples, target_length, n_features).
    """
    # Determine the target length
    if target_length is None:
        target_length = max(feature.shape[0] for feature in features)
    
    # Pad or truncate each feature array to the target length
    padded_features = []
    for feature in features:
        if feature.shape[0] < target_length:
            # Pad with zeros if the feature is shorter
            pad_width = target_length - feature.shape[0]
            padded_feature = np.pad(feature, ((0, pad_width), (0, 0)), mode='constant')
        else:
            # Truncate if the feature is longer
...
def create_folds(directory_audio, df):
    ...
    for file in directory_audio.rglob("*.wav"):
        ...
        # Manejar NaN o Inf en las características
        ...
        x_folds[fold - 1].append(feat)  # Mantener las características por archivo
...
def create_folds_original(directory_audio, df):
    """
...
def cargar_datos_original(trFolds, testFolds, global_vars):
